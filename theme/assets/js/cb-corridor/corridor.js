/* =========================================================================
   corridor.js â€” Coldwell Banker "CORRIDOR WALKTHROUGH" homepage (Home 7 preview)

   Extends the Home 4 fusion engine (Three.js dust-nebula + glass info-cards +
   custom cursor) with a scroll-driven CORRIDOR WALKTHROUGH walkthrough. The corridor is
   literally made of the dust: a third "STRUCT" particle pool lives in the SAME
   single Points draw call and, as you scroll the corridor act, condenses from
   scattered motes onto the edges of stylized corridor fixtures (threshold, reception,
   ...
   The camera dollies forward through The corridor, anchored to each section's own
   ...
   corridor assembles/dissolves (dirty flag); cards never use backdrop-filter.

   Progressive enhancement: init() adds html.cb-corridor-on only after WebGL starts;
   if the corridor build fails the nebula+cards still run (degrades to fusion); with
   no WebGL / reduced-motion the static glass-card fallback (cb-corridor.css) shows.
   Exposes window.CBCorridor.init(opts). Requires window.THREE.
   ========================================================================= */
(function (window, document) {
  'use strict';

  if (window.CBCorridor) { return; }
  ...
  // (z-12) with NO rotation â€” "looking around" is faked via x/y offsets.
  var W = [
  ...
  var corridorAssembly = 0, _structDirty = false, _camDirty = true;
  ...
  function parkStruct() {  // safe default before corridor build
  ...
  function defineCorridor() {
    ...
    // 4 (buyers beat) â€” a simple gateway frame so the corridor never feels empty
  ...
  function buildCorridorFromEdges() {
    // 1) merged LINE layer (manual merge â€” core three has no BufferGeometryUtils)
  ...
  function bakeMonogram(url, x, y, z, h) {
    ...
    img.onload = function () {
      try {
        ...
        // Render the OFFICIAL lockup in its own (white) color â€” do NOT recolor the
        // mark (BRAND.md Â§5/Â§9 forbid altering the lockup). AdditiveBlending over
  ...
  // ---- camera (per-section rect â†’ dolly) ----------------------------------
  function computeCamera() {
    if (!_corridorReady || !_sections.length) {
    ...
    // act progress from the whole sections container â†’ assembly with lead in/out
  ...
  function frame(t) {
    ...
    // Adaptive quality: sample frame time DURING the corridor phase (the heavy
    // render), not the idle hero; re-arm until it degrades once or stays healthy.
    if (!_degraded && corridorAssembly > 0.05) {
      _fpsAcc += dt; _fpsN++;
      if (_fpsN >= 90) {
    ...
    if (_corridorReady) {
      ...
      if (corridorAssembly > 0.001 || lineMat.uniforms.uReveal.value > 0.001) {
        lineMat.uniforms.uReveal.value += (corridorAssembly - lineMat.uniforms.uReveal.value) * 0.12;
        updateSignage();
      }
    }
    updateBursts(dt);
  ...
  function formCard(el, delay) {
    ...
    var isFrame = el.hasAttribute('data-corridor-frame');
  ...
  function countUp(el) {
    var nums = el.querySelectorAll('.cb-corridor-stat__num[data-count]'); if (!nums.length) { return; }
  ...
  function observeCards() {
    var cards = document.querySelectorAll('.cb-corridor-card[data-corridor]'); if (!cards.length) { return; }
  ...
  function observeSections() {
    var dots = document.querySelectorAll('.cb-corridor-nav__dot');
    var sections = document.querySelectorAll('[data-corridor-section]');
    if (sections.length && 'IntersectionObserver' in window && dots.length) {
      var sio = new IntersectionObserver(function (entries) {
        for (var i = 0; i < entries.length; i++) {
          if (entries[i].isIntersecting) {
            var idx = entries[i].target.getAttribute('data-corridor-section');
            for (var d = 0; d < dots.length; d++) { dots[d].classList.toggle('is-active', dots[d].getAttribute('data-corridor-to') === idx); }
          }
        }
    ...
    for (var k = 0; k < dots.length; k++) {
      dots[k].addEventListener('click', function () { var t = document.querySelector('[data-corridor-section="' + this.getAttribute('data-corridor-to') + '"]'); if (t) { t.scrollIntoView({ behavior: 'smooth', block: 'start' }); } });
    }
  }
  ...
  function init(opts) {
    ...
    var sel = opts.canvas || '#cb-corridor-canvas';
    ...
    try {
      ...
      renderer.setPixelRatio(dpr); renderer.setSize(vw, vh, false);
      document.documentElement.classList.add('cb-corridor-on');

      // corridor layer â€” isolated so a failure degrades to fusion (nebula+cards)
      try {
        _sections = [].slice.call(document.querySelectorAll('[data-corridor-section]'));
        _act = document.querySelector('.cb-corridor-scenes') || (_sections[0] && _sections[0].parentNode) || document.body;
        buildCorridor();
      } catch (eo) { _corridorReady = false; if (window.console) { console.warn('[cb-corridor] corridor build failed; running nebula+cards only.', eo); } }
      ...
      document.addEventListener('focusin', function (e) {
        var el = e.target, card = (el && el.closest) ? el.closest('.cb-corridor-card[data-corridor]') : null;
        if (card && card.__s !== 'formed') { formCard(card, 0); card.__s = 'formed'; }
      }, true);

      // Property Watch is decorative on the preview â€” stop the native page reload
      ...
      document.addEventListener('submit', function (e) {
        ...
        if (f && f.matches && f.matches('form[data-cb-watch]')) {
          ...
          if (f.parentNode) {
            ...
            note.className = 'cb-corridor-watch__note';
            note.style.opacity = '1';
            note.textContent = "Thanks â€” you're on the Property Watch list. We'll be in touch.";
      ...
      document.documentElement.classList.remove('cb-corridor-on'); stop();
      if (renderer && renderer.dispose) { try { renderer.dispose(); } catch (e3) {} }
      if (window.console) { console.warn('[cb-corridor] init failed; using static fallback.', err); }
      return false;
    }
  }

  window.CBCorridor = { init: init };
